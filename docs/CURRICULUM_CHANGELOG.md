# Curriculum Changelog

## v1-preaudit-2025-12-23
- Date: 2025-12-23
- Initial pre-audit version stamp.

## v1-preaudit-2025-12-23c
- Rehydrated CAT, CODA, Relapse Prevention, and Anger Management sessions with scaffolded data based on local file artifacts.
- Verified session counts: Prime (16), DBT (48), CAT (9), CODA (10), Relapse Prevention (9), Anger Management (12).

## v1-preaudit-2025-12-24f
- CAT Session 1: participant activity content (worksheet + scenarios) visible without facilitator launch.

## v1-preaudit-2025-12-24e
- CAT Session 1 replaced with verbatim facilitator + participant content, worksheet, scenarios, and CaseWorx export template.

## v1-preaudit-2025-12-24c
- CAT Session 4: participant view no longer gated by activity launch.
- CaseWorx note template updated for court-ready export.

## v1-preaudit-2025-12-24-fulltext
- RESTORED CAT Session 4 to "FULL VERBATIM" long-form text (exactly as authored).
- Updated Facilitator UI to render 'responses', 'notes', and 'caseworx' guidance.
- Fixed UI text wrapping to preserve original line breaks and formatting.
- Added new FacilitatorPrompt types to schema.

## v1-preaudit-2025-12-24-gold
- RESTORED CAT Session 4 ("Core Beliefs") to "Gold Session" standard with LOCKED INTENT.
- Restored procedural/engagement tone in Opening.
- Restored accountability framing in Review.
- Locked section intents with code comments.
- This session is now the rigid template for CAT session tone.

## v1-preaudit-2025-12-23b
- Date: 2025-12-23
- Rehydrated curricula sessions into file-based runtime (DBT wired; other sources missing).

## v1-preaudit-2025-12-24-cat-batch2
- CAT Sessions 6–7 installed (scaffold). Worksheet/scenario headings visible to participants without launch.

## v1-preaudit-2025-12-24-cat-batch3
- CAT Sessions 8–9 installed (scaffold). Worksheet/scenario headings visible to participants without launch.

## v1-preaudit-2025-12-25-rp-import1
- Relapse Prevention Sessions 1–9 imported from RP_S01–RP_S09 markdowns (scaffold).
