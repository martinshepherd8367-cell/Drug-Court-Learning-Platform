          ...
          {selectedClass && (
            <div className="space-y-4">
              ...
              <div className="border-t pt-4">
                ...
                <div className="space-y-2">
                  {(selectedClass.roster || []).map((p:any, i:number) => (
                      <div key={i} className="flex items-center justify-between p-2 bg-gray-50 rounded">
                        <div>
                          <div className="font-medium text-sm">{p.name}</div>
                          <div className="text-xs text-gray-500">Session {selectedClass.session}</div>
                        </div>
                        <Badge className="bg-green-100 text-green-800">Active</Badge>
                      </div>
                    ))}
                  {(selectedClass.roster || []).length === 0 && <span className="text-gray-500 text-sm">No participants assigned</span>}
              </div>
