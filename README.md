...
A role-based web application designed to support accountability court participants and facilitators during structured psychoeducational programs. Built with Next.js 16, React 19, TypeScript, and Tailwind CSS.
...
This application is the **Golden Baseline** and must only be deployed to:
- **Service**: `drug-court-learning-platform-458193648844`
- **Region**: `us-central1`

**Deployment Rule:**
Using `scripts/deploy-central.sh` is the only supported deployment method. Deploying elsewhere or using manual `gcloud` commands without these specific parameters risks overwriting the wrong application or violating regional constraints.
...
- Tailwind CSS
- Hybrid Architecture:
  - React Context for client UI state
  - Firebase Auth + Firestore for Admin Ops & Authentication
- shadcn/ui components

---

## Current Status (Hybrid V0 Build)

- **Admin Daily Ops Cockpit**: Fully wired to Firestore (real-time data).
- **Authentication**: Firebase Auth enforced (DEV_MODE disabled).
- **UI Scaffolding**: Admin/Facilitator/Participant dashboards implemented.
- **Schedule**: Wired to `Enrollment` data (mock or seeded).
- **Seed Data**: Automated seeding via `/api/admin/seed` supporting "Daily Events" model.
- **Curriculum**: Store-driven architecture (mock data source).
- **CaseWorx Export**: Manual copy/paste; automation is future work.
...
// Schedule info (V0 implementation)
